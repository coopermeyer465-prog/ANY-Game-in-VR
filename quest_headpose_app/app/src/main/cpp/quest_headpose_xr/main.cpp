...
void NotifyOpenXrReady(android_app* app, const std::string& message) {
    ...
    jmethodID readyMethod = env->GetMethodID(activityClass, "onNativeOpenXrReady", "(Ljava/lang/String;)V");
    if (readyMethod != nullptr) {
        jstring status = env->NewStringUTF(message.c_str());
        env->CallVoidMethod(app->activity->clazz, readyMethod, status);
        env->DeleteLocalRef(status);
    }
    env->DeleteLocalRef(activityClass);
}

void NotifyOpenXrError(android_app* app, const std::string& message) {
    JNIEnv* env = nullptr;
    app->activity->vm->AttachCurrentThread(&env, nullptr);
    jclass activityClass = env->GetObjectClass(app->activity->clazz);
    jmethodID errorMethod = env->GetMethodID(activityClass, "onNativeOpenXrError", "(Ljava/lang/String;)V");
    if (errorMethod != nullptr) {
        jstring status = env->NewStringUTF(message.c_str());
        env->CallVoidMethod(app->activity->clazz, errorMethod, status);
        env->DeleteLocalRef(status);
    }
    env->DeleteLocalRef(activityClass);
}
...
void android_main(struct android_app* app) {
    try {
        ...
        NotifyOpenXrReady(app, "OpenXR session active");
        ...
        NotifyOpenXrError(app, ex.what());
    } catch (...) {
        Log::Write(Log::Level::Error, "Unknown Error");
        NotifyOpenXrError(app, "Unknown OpenXR error");
    }
}
#else
